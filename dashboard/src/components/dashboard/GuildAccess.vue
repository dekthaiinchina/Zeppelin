<template>
  <div>
    ...
    <p>On this page you can manage who has access to the server's Zeppelin dashboard.</p>

    <h2 class="mt-8">Roles</h2>
    <ul>
      <li><strong>Owner:</strong> All permissions. Managed automatically by the bot.</li>
      <li>
        <strong>Bot manager:</strong> Can manage dashboard users (including other bot managers) and edit server
        configuration
      </li>
    ...
    <div class="mt-4">
      <div v-if="permanentPermissionAssignments.length === 0">No dashboard users</div>
        ...
        <li v-for="perm in permanentPermissionAssignments">
          <div class="flex gap-4">
            ...
            <div class="flex gap-4">
              ...
              <a href="#" v-on:click="deletePermissionAssignment(perm)" v-if="!isOwner(perm)"> Delete </a>
            </div>
          </div>
        </li>
      </ul>
      <div class="mt-2">
        <a href="#" v-on:click="addPermissionAssignment()"> Add new user </a>
      </div>
    </div>
    ...
    <div v-if="temporaryPermissionAssignments.length === 0">No temporary dashboard users</div>
    <ul v-if="temporaryPermissionAssignments.length">
      <li v-for="perm in temporaryPermissionAssignments">
        <div class="flex gap-4">
          ...
          <div>Expires in {{ formatTimeRemaining(perm) }}</div>
          <div>
            <a href="#" v-on:click="deletePermissionAssignment(perm)"> Delete </a>
          </div>
        </div>
      </li>
    </ul>
    <div class="mt-2">
      <a href="#" v-on:click="addTemporaryPermissionAssignment()"> Add temporary user for 1 hour </a>
    </div>
  </div>
</template>
