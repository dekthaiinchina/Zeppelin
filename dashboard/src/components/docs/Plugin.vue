  ...
  <div v-else>
    ...
    <div v-if="data.info.type === 'legacy'">
      <div class="px-3 py-2 mb-4 rounded bg-gray-800 shadow-md flex">
        ...
        <div class="flex-auto">
          <strong>Note!</strong> This is a legacy plugin which is no longer actively maintained and may be removed in a
          future update.
        </div>
      </div>
    </div>
    ...
    <Tabs>
      ...
      <Tab :active="tab === 'configuration'">
        <router-link class="unstyled" v-bind:to="'/docs/plugins/' + pluginName + '/configuration'"
    ...
    <div class="usage" v-if="tab === 'usage'">
      ...
      <div v-if="!hasUsageInfo">
        This plugin has no usage information. See
      ...
      <div v-if="data.messageCommands && data.messageCommands.length">
        ...
        <div
          v-for="command in data.messageCommands || []"
          class="command mb-4"
          v-bind:ref="getCommandSlug(command)"
          v-bind:class="{ target: targetCommandId === getCommandSlug(command) }"
        >
          <h4 class="text-xl font-semibold mb-0">
            <span v-for="(trigger, index) in getTriggers(command)">
              <span class="text-gray-600" v-if="typeof index === 'number' && index > 0">/</span> !{{ trigger }}
            </span>
          </h4>
          ...
          <Expandable class="mt-4">
            ...
            <template v-slot:content>
              ...
              <div v-if="command.signature">
                ...
                <ul class="list-none">
                  <li
                    v-for="(signature, index) in getCommandSignatures(command)"
                    v-bind:class="{ 'mt-8': typeof index === 'number' && index !== 0 }"
                  >
                    <code class="inline-code bg-gray-900">
                      !{{ getTriggers(command)[0] }}
                      <span v-for="paramInfo in getSignatureParameters(signature)"
                        >{{ renderParameterOrOption(paramInfo.name, paramInfo.param) }}
                      </span>
                    </code>
                      ...
                      <div v-if="getSignatureParameters(signature).length">
                        ...
                        <ul>
                          <li v-for="paramInfo in getSignatureParameters(signature)">
                            ...
                            <router-link :to="'/docs/reference/argument-types#' + (paramInfo.param.type || 'string')">{{
                              paramInfo.param.type || "string"
                            }}</router-link>
                            <MarkdownBlock
                              v-if="paramInfo.param.description"
                      ...
                      <div v-if="getSignatureOptions(signature).length">
                        ...
                        <ul>
                          <li v-for="optionInfo in getSignatureOptions(signature)">
                            ...
                            <router-link
                              :to="'/docs/reference/argument-types#' + (optionInfo.option.type || 'string')"
                              >{{ optionInfo.option.type || "string" }}</router-link
                            >
                            <MarkdownBlock
                              v-if="optionInfo.option.description"
    ...
    <div class="configuration" v-if="tab === 'configuration'">
      <!-- Basic config info -->
      <p>
        <strong>Name in config:</strong> <code>{{ data.name }}</code
        ><br />
        To enable this plugin with default configuration, add <code>{{ data.name }}: {}</code> to the
        <code>plugins</code> list in config
      </p>
